...
class Visualizer:
    ...
    def animate_movement(self, position_data, front_distances, 
        ...
        # Create text annotations for sensor distances
        front_text = self.train_axis.text(
            0.05, 0.95, '', transform=self.train_axis.transAxes, 
            verticalalignment='top', fontsize=10, color='black'
        )
        right_text = self.train_axis.text(
            0.05, 0.90, '', transform=self.train_axis.transAxes, 
            verticalalignment='top', fontsize=10, color='black'
        )
        left_text = self.train_axis.text(
        ...
        def update_frame(frame_index):
            ...
            front_text.set_text(f'Front Distance: {front_distances[frame_index]:.2f}')
            right_text.set_text(f'Right Distance: {right_distances[frame_index]:.2f}')
            left_text.set_text(f'Left Distance: {left_distances[frame_index]:.2f}')
